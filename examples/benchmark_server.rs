...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("client") => {
            let connections = args.get(2)
                .and_then(|s| s.parse::<usize>().ok())
                .unwrap_or(100);
            let requests_per_conn = args.get(3)
                .and_then(|s| s.parse::<usize>().ok())
                .unwrap_or(1000);
            run_client(connections, requests_per_conn).await?
        },
        _ => {
            println!("用法: cargo run --release --example benchmark_server -- [server|client] [连接数] [每连接请求数]");
...
async fn run_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stats_handle = tokio::spawn(async move {
        ...
        loop {
            ...
            println!("[Stats] 当前RPS: {:.2} req/s, 总请求数: {}", rps, current_count);
...
/// 运行客户端基准测试
async fn run_client(connections: usize, requests_per_conn: usize) -> Result<(), Box<dyn std::error::Error>> {
    println!("[Client] 开始基准测试: {} 并发连接, 每连接 {} 请求", connections, requests_per_conn);
    ...
    let progress_handle = tokio::spawn(async move {
        loop {
            ...
            println!("[Progress] {:.2}% ({}/{})", progress, completed, total_requests);
    ...
    println!("吞吐量: {:.2} 请求/秒", completed as f64 / elapsed.as_secs_f64());
